...
# DHIS2 with docker
Docker makes core-teams development and testing efforts easier every day! It allows us to spin up various environments with any DHIS2 version, numerous tomcat versions and different components, such as redis, nginx, in no time. Because we wanted to share docker with community, we made publishing docker images a part of our delivery pipeline. This guide aims to provide some guidelines on how to use docker to quickly set up DHIS2. 
...
##### dev
Repositories suffixed with `-dev` contains images for _latest_ DHIS2 versions. That means, that we can't fully guarantee its quality and we recommend to use it only for testing purposes. Also, for curious souls, wanting to peek into the latest DHIS2 development efforts. The `-dev` repositories are ideologically the same as `dev` demo instances on _play_. Images in this channel will be overwritten multiple times a day. 
